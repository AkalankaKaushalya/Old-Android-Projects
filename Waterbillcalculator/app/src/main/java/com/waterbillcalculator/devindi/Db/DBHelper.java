package com.waterbillcalculator.devindi.Db;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase DB) {
        DB.execSQL("create Table Wattb(BilNo TEXT PRIMARY KEY,  lbdate TEXT, nbdate TEXT, lunit TEXT, nunit TEXT, Nuseunit TEXT, oneUnitP TEXT, ThisMonPris TEXT)");

    }

    @Override
    public void onUpgrade(SQLiteDatabase DB, int i, int i1) {
        DB.execSQL("drop Table if exists Wattb");
    ...
    public boolean saveData(String blno, String Lbdate, String Nbdate, String Lunit, String Nunit, String nowUunit, String Oneupris, String tiseMpris) {
        ...
        long resalt = DB.insert("Wattb", null, contentValues);
        if (resalt == 1) {
            return false;
        } else {
            return true;
    ...
    public boolean datadelete(String bilno) {
        SQLiteDatabase DB = this.getWritableDatabase();
        Cursor cursor = DB.rawQuery("Select * from Wattb where bilno = ?", new String[]{bilno});// ඩිලිට් මෙතඩ් එකදී කසර් ලයිබ්‍රි එක ඉන්පොර්ට් කරගන්නවා
        if (cursor.getCount() > 0) {
            long resalt = DB.delete("Wattb", "BilNo=?", new String[]{bilno});// අපි මෙකෙදී ඩිලිට් කරන්නෙ අපි යොදගත් ප්‍රිමරි කී එකයි
            if (resalt == -1) {
                return false;
            } else {
                return true;
    ...
    public boolean dataupdate(String blno, String Lbdate, String Nbdate, String Lunit, String Nunit, String nowUunit, String Oneupris, String tiseMpris) {
        ...
        contentValues.put("oneUnitP", Oneupris);
        contentValues.put("ThisMonPris", tiseMpris);
        Cursor cursor = DB.rawQuery("Select * from Wattb where BilNo = ?", new String[]{blno});
        if (cursor.getCount() > 0) {
            long resalt = DB.update("Wattb", contentValues, "BilNo=?", new String[]{blno});
            if (resalt == -1) {
    ...
    public Cursor getNotes() {
        SQLiteDatabase DB = this.getWritableDatabase();
        Cursor cursor = DB.rawQuery("Select * from Wattb", null);
        return cursor;
    }
